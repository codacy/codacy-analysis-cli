...
class DuplicationToolSpec extends Specification with NoLanguageFeatures {
        ...
        (_, directory) =>
          val expectedClones = Seq(
            DuplicationClone(
              ...
              18,
              Set(
                DuplicationCloneFile("test2.js", 13, 30),
                DuplicationCloneFile("test.js", 54, 71))),
            DuplicationClone(
              "",
              69,
              22,
  ...
  def assertDuplication(
    duplicationResults: Set[DuplicationClone],
    expectedClones: Seq[DuplicationClone]): MatchResult[Set[DuplicationClone]] = {
    //ignore the clone lines field for assertion
    duplicationResults.map(_.files.to[Set]) must containTheSameElementsAs(
      expectedClones.map(_.files.to[Set]))
