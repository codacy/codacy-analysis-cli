...
class AnalyseExecutor(formatter: Formatter,
  ...
  def run(): Either[CLIError, Seq[ExecutorResult[_]]] = {
    ...
    val filesTargetAndTool: Either[CLIError, (FilesTarget, FilesTarget, Set[ITool])] = for {
      ...
      filesGlobalTarget = fileCollector
        .filterGlobal(allFilesTarget, configuration.fileExclusionRules)
      tools <- allTools(
        configuration.tool,
        configuration.toolConfiguration,
        LanguagesHelper.fromFileTarget(
          filesGlobalTarget,
          configuration.fileExclusionRules.allowedExtensionsByLanguage))
            ...
            tool match {
              ...
              case metricsTool: MetricsTool =>
                ...
                MetricsToolExecutorResult(
                  metricsTool.languageToRun.name,
  ...
  private def postProcess(analysisResult: Either[CLIError, Seq[ExecutorResult[_]]])
    : Either[CLIError, Seq[ExecutorResult[_]]] = {
    analysisResult.map { result =>
      val (issuesResults, duplicationResults, metricsResults) =
        result.partitionSubtypes[
          IssuesToolExecutorResult,
          DuplicationToolExecutorResult,
          MetricsToolExecutorResult]

      val reduce = MetricsToolExecutor.reduceMetricsToolResultsByFile _
      val calculateMissingMetrics
        : Seq[MetricsToolExecutorResult] => Seq[MetricsToolExecutorResult] =
      ...
      executorResults.foreach(
        _.analysisResults.foreach(results => formatter.addAll(results.to[List])))
      formatter.end()
  ...
  private def getToolConfiguration(tool: Tool,
    ...
    (for {
      ...
      if (!shouldRun) {
        ...
        Success(
          CodacyCfg(toolConfiguration.patterns.map(toInternalPattern), baseSubDir, extraValues))
      }
  ...
  private def getExtraConfiguration(
    enginesConfiguration: Option[Map[String, CLIConfiguration.IssuesTool.Extra]],
...
object AnalyseExecutor {
  ...
  final case class IssuesToolExecutorResult(toolName: String,
                                            files: Set[Path],
                                            analysisResults: Try[Set[ToolResult]])
      extends ExecutorResult[ToolResult]

  final case class MetricsToolExecutorResult(language: String,
                                             files: Set[Path],
                                             analysisResults: Try[Set[FileMetrics]])
  ...
  def tools(toolInput: Option[String],
    ...
    def fromRemoteConfig: Either[CLIError, Set[Tool]] = {
      configuration.toolConfigurations.left.map(CLIError.NoRemoteProjectConfiguration).flatMap {
        toolConfiguration =>
