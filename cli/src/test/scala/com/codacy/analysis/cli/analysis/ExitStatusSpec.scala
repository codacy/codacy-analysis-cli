...
class ExitStatusSpec
  ...
  "ExitStatus" should {
    ...
    "send failed analysis code" in {
      new ExitStatus(3).exitCode(Left(CLIError.FilesAccessError)) should beEqualTo(
      ...
      new ExitStatus(2).exitCode(Right(Seq(IssuesToolExecutorResult(
        ...
        Success(Set(
          ...
          Issue(
            ...
            Result.Level.Err,
            Option(Pattern.Category.ErrorProne),
            FullLocation(3, 20)))))))) should beEqualTo(
        ExitStatus.ExitCodes.maxAllowedIssuesExceeded)
    }

    "send success code when no issues" in {
      new ExitStatus(10).exitCode(
        Right(Seq(IssuesToolExecutorResult(
          "MyTool",
          Set(Paths.get("Test.scala")),
          Success(Set()))))) should beEqualTo(ExitStatus.ExitCodes.success)
    }
    ...
    "send partial failure when some tools fail" in {
      new ExitStatus(10, failIfIncomplete = true).exitCode(Right(Seq(
        ...
        IssuesToolExecutorResult(
          "MyTool",
          Set(Paths.get("Test.scala")),
          Failure(new Exception("Failed")))))) should beEqualTo(
    ...
    "send failedUpload when uploader has an error" in {
      new ExitStatus(10, failIfIncomplete = true)
        .exitCode(Left(CLIError.UploadError("Failed upload!"))) should beEqualTo(
        ExitStatus.ExitCodes.failedUpload)
    }

    "send nonExistentTool when analysis fails because of non-existent tool argument value" in {
      new ExitStatus(10, failIfIncomplete = true)
        .exitCode(Left(CLIError.NonExistingToolInput("tool"))) should beEqualTo(
    ...
    "send commit uuids do not match exit code when the commit uuid from git did not match the cli argument" in {
      new ExitStatus(10).exitCode(Left(
        CLIError.CommitUuidsDoNotMatch(Commit.Uuid("arg"), Commit.Uuid("git")))) should beEqualTo(
