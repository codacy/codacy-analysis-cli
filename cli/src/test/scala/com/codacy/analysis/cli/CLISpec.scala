...
class CLISpec extends Specification with NoLanguageFeatures with FileMatchers {
  ...
  "CLIApp" should {
    ...
    "output correct duplication" in {
      withClonedRepo(
        "git://github.com/qamine-test/duplication-delta.git",
        "625e19cd9be4898939a7c40dbeb2b17e40df9d54") { (file, directory) =>
        cli.main(
          Array(
            "analyse",
            "--directory",
      ...
      withTemporaryGitRepo { directory =>
        (for {
          ...
          val analyse = Analyse(
            options = CommonOptions(),
            api = APIOptions(
              projectToken = Some("hey, im a token"),
              codacyApiBaseUrl = Some("https://codacy.com")),
            tool = None,
            directory = Option(directory),
            upload = Tag.of(0),
            extras = ExtraOptions(),
  ...
  private def errorMsg(message: String)
    : (DefaultCommand,
       List[String],
       Option[Either[String, (String, CommandOptions, Seq[String], Seq[String])]]) = {
