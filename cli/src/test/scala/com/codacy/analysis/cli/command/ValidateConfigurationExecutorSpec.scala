...
class ValidateConfigurationExecutorSpec extends Specification with NoLanguageFeatures with FileMatchers {

  "ValidateConfigurationExecutor" should {
    "find configuration file" in {
      File.temporaryDirectory().map { directory =>
        val resource = Resource.getAsString("com/codacy/analysis/core/configuration/codacy.yaml")
        (directory / ".codacy.yaml").write(resource)

        val command = new ValidateConfigurationExecutor(ValidateConfiguration(CommonOptions(), Option(directory)))
